...
class OperatorBlock(nn.Module):
    def __init__(self, n_channels, modes1, modes2, activation):
        ...
        self.conv = SpectralConv2d(
            self.n_channels, self.n_channels, self.modes1, self.modes2
        )
...
class DSFNO(nn.Module):
    ...
    def forward(self, x, upsample_factor):
        ...
        out = torch.nn.functional.interpolate(
            out, scale_factor=upsample_factor, mode="bicubic", align_corners=False
        )
        ...
        out = self.fc1(out)  # linear layer
        out = F.gelu(out)
        # (n_batch, upsample_factor * size_x, upsample_factor * size_y, 128)

        out = self.fc2(out)  # 2nd linear layer
