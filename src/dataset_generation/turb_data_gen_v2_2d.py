...
def downaverage_state(state: jnp.ndarray, downsample_factor: int) -> jnp.ndarray:
    ...
    if h_in % downsample_factor != 0 or w_in % downsample_factor != 0:
        raise ValueError(
    ...
    reshaped = state.reshape(
        num_vars, h_out, downsample_factor, w_out, downsample_factor
    )
...
h5_path = os.path.join(save_path, "2d_states.h5")
...
while i < max_sims:
    ...
    if jnp.isnan(result_high_res.states).any():
        continue
    ...
    if jnp.isnan(result_low_res.states).any():
        continue

    for snapshot_idx, snapshot in enumerate(result_high_res.states[1:-1]):
