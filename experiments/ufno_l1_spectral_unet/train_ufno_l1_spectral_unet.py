"""
UFNO U-Net depth sweep for CFNO shift=8 with L1 + spectral loss (w_minor).

This single self-contained script trains four ``UFNO_2`` (normalized) models
that augment the FNO_2 operator blocks with a parallel 3-D U-Net path (per
https://github.com/RuneRost/RT-JAX/blob/main/architectures/ufno_3d.py).  All
loss/training hyper-parameters mirror the l1_spectral_skip_connection
reference run (``cfno_shift8_skip_trilinear``) using the ``w_minor`` spectral
weight from ``calibration.json``; only ``n_operator_blocks`` in {1, 2} and
``dropout_rate`` in {0.0, 0.1} vary across the four runs.

After training, the script:

1. Evaluates the four trained UFNO runs PLUS the prior best l1_spectral
...
2. Writes ``benchmark_metrics.csv`` and emits grouped bar-chart PNGs
   comparing the runs at each eval scale (x4 and x2) — saved DIRECTLY into
   the repo experiment dir (``experiments/ufno_l1_spectral_unet/``), NOT into
   the scratch output dir.  Weights / losses / config / manifest stay in
   scratch.
3. Runs a fresh jf1uids turbulent simulation (seed 1234, 128^3), block-averages
   ...
   comparison (HR | UFNO variants | skip=False ref | Trilinear | LR).
   States are cached to ``comparison_states.npy`` (in scratch).

Usage
-----
    bash experiments/ufno_l1_spectral_unet/run.sh
    python experiments/ufno_l1_spectral_unet/train_ufno_l1_spectral_unet.py
    python experiments/ufno_l1_spectral_unet/train_ufno_l1_spectral_unet.py \
...
FOLDER_NAME = "ufno_l1_spectral_unet_"
REPO_EXP_DIR = ROOT / "experiments" / "ufno_l1_spectral_unet"
...
# ── UFNO hyper-parameters (shift=8) ────────────────────────────────────
# n_operator_blocks and dropout_rate vary per run (the sweep axes); they are
# NOT in UFNO_PARAMS but passed explicitly in _build_model / _build_run_grid.
...
def _build_run_grid(w_minor: float) -> list[dict]:
    """Four trained UFNO runs: 2x2 grid over n_operator_blocks x dropout_rate."""
    base = {
        "model_type": "ufno",
    ...
    for n_op in (1, 2):
        for drop in (0.0, 0.1):
            drop_tag = "0" if drop == 0.0 else "01"
            grid.append(
                {
                    **base,
                    "name": f"ufno_shift8_op{n_op}_drop{drop_tag}",
                    "label": f"UFNO op={n_op} drop={drop:g}",
                    "n_operator_blocks": n_op,
                    "dropout_rate": drop,
                }
            )
    return grid
...
def _build_model(run_config: dict) -> nn.Module:
    """Instantiate UFNO_2 with the run's n_operator_blocks / dropout_rate."""
    return UFNO_2(
        **UFNO_PARAMS,
...
def _write_manifest(output_dir: Path, runs: list[dict], w_minor: float) -> dict:
    ...
    for run in runs:
        params = {
            **UFNO_PARAMS,
            ...
            "n_operator_blocks": run["n_operator_blocks"],
    ...
    manifest = {
        "experiment": "ufno_l1_spectral_unet",
        "output_dir": str(output_dir),
        "benchmark_csv": "experiments/ufno_l1_spectral_unet/benchmark_metrics.csv",
    ...
    for entry in manifest["models"]:
        ...
        try:
            ...
            for scale in entry.get("eval_scales", [UPSAMPLE_FACTOR]):
                ...
                res["n_operator_blocks"] = entry.get("n_operator_blocks")
                res["dropout_rate"] = entry.get("dropout_rate")
                rows.append(res)
...
# Each group: (filename stem, title template with {scale}, metric specs).
# _plot_bar_charts emits one PNG per scale present in the eval data, naming
# them ``<stem>_x<scale>.png`` (e.g. bar_mse_channel_x4.png, ..._x2.png).
_BAR_GROUPS = [
    (
        "bar_mse_channel",
        "UFNO L1+spectral: MSE & per-channel (x{scale})",
        [
            ("MSE", "↓"),
            ("Loss_density", "↓"),
            ("Loss_pressure", "↓"),
    ...
    (
        "bar_spectral_perceptual_psnr_ssim",
        "UFNO L1+spectral: spectral / perceptual / PSNR / SSIM (x{scale})",
        [
            ("Spectral_MSE", "↓"),
            ("Perceptual", "↓"),
            ("PSNR", "↑"),
...
def _plot_one_bar_group(
    ...
    present = [m for m, _ in metrics if m in sub.columns]
    if not present:
        return
    n = len(present)
    n_cols = min(4, n)
    ...
    fig.suptitle(title_tmpl.format(scale=scale), fontsize=13, y=0.98)
    fig.tight_layout(rect=[0, 0.06, 1, 0.96])
    out = output_dir / f"{stem}_x{scale}.png"
...
def _plot_bar_charts(eval_df: pd.DataFrame, output_dir: Path) -> None:
    label_col = "model"
    for scale in _BAR_SCALES:
        sub = eval_df[eval_df["upsample_factor"] == scale].copy()
        if sub.empty:
            print(f"  No x{scale} rows — skipping bar charts for this scale")
            continue
        labels = sub[label_col].tolist()
        colors = plt.cm.tab10(np.linspace(0, 0.9, len(labels)))
        for stem, title_tmpl, metrics in _BAR_GROUPS:
            _plot_one_bar_group(
                sub, labels, colors, stem, title_tmpl, metrics, scale, output_dir
            )
...
def _plot_final_snapshot(states: dict, manifest: dict, output_dir: Path) -> None:
    ...
    _plot_state_grid(
        ...
        title="UFNO shift=8 L1+spectral — x4: U-Net depth sweep",
    )
...
def _run_eval_and_plots(manifest: dict, output_dir: Path, regen: bool) -> None:
    ...
    eval_df.to_csv(repo_csv, index=False)
    print(f"  Saved {repo_csv}")

    _plot_bar_charts(eval_df, REPO_EXP_DIR)

    print("\nBuilding final-snapshot comparison …")
    torch.manual_seed(SEED)
    # comparison_states.npy stays in scratch (large artifact); only the
    # final PNG is routed to the repo experiment dir.
