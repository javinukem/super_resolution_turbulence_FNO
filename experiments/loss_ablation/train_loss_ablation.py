"""
Single-loss ablation for CFNO shift=8 (normalized).

This single self-contained script trains three CFNO shift=8 (normalized)
models that are identical in architecture, data, and training schedule,
differing ONLY in the loss function used during training:

1. ``l1``       — ``nn.L1Loss()`` (mean absolute error)
2. ``mse``      — ``nn.MSELoss()`` (mean squared error)
3. ``spectral`` — velocity-only torch-FFT log-power spectral loss
                  (``SpectralLoss``, pool=2; no L1/MSE term)

All CFNO hyper-parameters, normalization stats, dataset, snapshot index,
optimizer, scheduler, and early-stopping settings mirror
``experiments/l1_spectral_skip_connection/train_l1_spectral_skip_connection.py``
so the three runs are directly comparable.  The skip-connection branch is
active for every run (``skip_connection=True``, ``interpolation_mode=
"trilinear"``, ``skip_connection_interpolation_mode="trilinear"``) — the
only varying axis is the loss.
...
2. Writes ``benchmark_metrics.csv`` (scratch + repo copy) and emits two
   grouped bar-chart PNGs comparing the three runs at x4.  The bar charts
   contain ONLY the three trained models — no trilinear interpolation
   benchmark and no adopted reference model.
3. Runs a fresh jf1uids turbulent simulation (seed 1234, 128^3), block-averages
   ...
   comparison (HR | L1 only | MSE only | Spectral only | Trilinear | LR).
   States are cached to ``comparison_states.npy``.

Usage
-----
    bash experiments/loss_ablation/run.sh
    python experiments/loss_ablation/train_loss_ablation.py
    python experiments/loss_ablation/train_loss_ablation.py \
...
FOLDER_NAME = "loss_ablation_"
REPO_EXP_DIR = ROOT / "experiments" / "loss_ablation"
REPO_EXP_DIR.mkdir(parents=True, exist_ok=True)

# Reuse the l1_spectral_weighting normalization stats (do NOT regenerate) so
# results are directly comparable to that experiment.
...
# ── Training hyper-parameters (mirror l1_spectral_skip_connection) ────
...
# ── Skip-connection config (fixed for all 3 runs; loss is the only axis) ──
...
def _build_loss_fn(loss_type: str, vx: int, vy: int, vz: int) -> nn.Module:
    """Return a single-loss module whose forward returns (loss, metrics_dict)."""
    if loss_type == "l1":
        return _ScalarLoss(nn.L1Loss())
    if loss_type == "mse":
        return _ScalarLoss(nn.MSELoss())
    if loss_type == "spectral":
        return SpectralLoss(vx, vy, vz)
...
def _build_run_grid() -> list[dict]:
    """Three trained runs: identical except for the loss function."""
    base = {
        "model_type": "cfno",
    ...
    return [
        {
            **base,
            "name": "cfno_shift8_l1_only",
            "label": "L1 only",
            "loss": "l1",
        },
        {
            **base,
            "name": "cfno_shift8_mse_only",
            "label": "MSE only",
            "loss": "mse",
        },
        {
            **base,
            "name": "cfno_shift8_spectral_only",
            "label": "Spectral only",
            "loss": "spectral",
        },
    ]
...
def _save_artifacts(
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title(f"{run_config['name']} — loss={run_config['loss']}")
    ax.legend()
    ax.set_yscale("log")
    ...
    loss_block = {"type": run_config["loss"]}
    if run_config["loss"] == "spectral":
        loss_block["spectral_pool"] = SPECTRAL_POOL
...
def train_one_run(
    ...
    loss_fn = _build_loss_fn(run_config["loss"], vx, vy, vz)
...
def _write_manifest(output_dir: Path, runs: list[dict]) -> dict:
    """Manifest with ONLY the three trained cfno models (no trilinear, no ref)."""
    models = []
    for run in runs:
        params = {
            **CFNO_PARAMS,
    ...
    manifest = {
        ...
        "benchmark_csv": "experiments/loss_ablation/benchmark_metrics.csv",
...
_BAR_GROUPS = [
    (
        "bar_mse_channel.png",
        "Loss ablation (CFNO shift=8): MSE & per-channel (x4)",
        [
            ("MSE", "↓"),
            ("Loss_density", "↓"),
            ("Loss_pressure", "↓"),
    ...
    (
        ...
        "Loss ablation (CFNO shift=8): spectral / perceptual / PSNR / SSIM (x4)",
        [
            ("Spectral_MSE", "↓"),
            ("Perceptual", "↓"),
            ("PSNR", "↑"),
...
def _plot_bar_charts(eval_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for fname, title, metrics in _BAR_GROUPS:
        ...
        if handles:
            fig.legend(
                handles, hlabels, loc="lower center", ncol=min(len(labels), 5),
                fontsize=8,
            )
...
def _build_states(
    regen: bool, manifest: dict, states_npy: Path
) -> dict:
...
def _plot_final_snapshot(states: dict, manifest: dict, output_dir: Path) -> None:
    ...
    _plot_state_grid(
        ...
        title="CFNO shift=8 — x4: single-loss comparison (L1 / MSE / spectral)",
    )
...
def main() -> None:
    ...
    rv = _get_registered_variables_3d()
    print(f"  velocity_index: x={rv.velocity_index.x} y={rv.velocity_index.y} z={rv.velocity_index.z}")
    ...
    for r in runs:
        print(
            ...
            f"interp={r['interpolation_mode']}  "
            f"skip={r['skip_connection']}"
        )
    ...
    for run_config in runs:
        ...
        try:
            best_val = train_one_run(
                run_config, train_loader, val_loader, output_dir
            )
            results[run_config["name"]] = {
                "status": "success",
