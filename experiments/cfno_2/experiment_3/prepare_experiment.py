...
from experiments.cfno_2.experiment_3.fno_2_exp_2 import FNO_2
...
parent_experiment_folder = os.path.abspath(
    "/export/data/jalegria/experiments/experiment_3"
)


def train_on_gpu(config_dict, use_normalizing=True, append_vars: str = ""):
    if not os.path.isdir(parent_experiment_folder):
        raise FileNotFoundError(f"folder {parent_experiment_folder} not found")
    config = config_dict
    start = time.time()
    dataset = dataset_sr(
        max_samples=config["training"]["max_samples"], use_normalizing=use_normalizing
    )
    ...
    test_losses, train_losses, model = training_model(
        ...
        accumulate_gradients=config["training"]["accumulate_gradients"],
        print_updates=True,
    )
    ...
    model_vars = (
        ...
        f"lk{config['fno_2']['last_layer_kernel']}_"
        f"lc{config['fno_2']['last_layer_constraint']}_" + append_vars
    )
    # date_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    exp_folder = os.path.join(parent_experiment_folder, f"{model_vars}")
    ...
    print(f"Finished training {model_vars}")
