...
def create_dataset():
    dataset = dataset_sr(snapshot_index=79, use_normalizing=True)
...
def objective_parent(
    ...
    def objective(trial: trial.Trial):
        ...
        # --- Create model ONCE on CPU ---
        model = FNO_2(
            in_channel=5,
            n_channels=params["n_channels"],
            n_residual_blocks=params["n_residual_blocks"],
            n_operator_blocks=params["n_operator_blocks"],
            modes=params["modes"],
            shifting_modes=params["shifting_modes"],
            last_layer_constraint=params["last_layer_constraint"],
        )

        param_count = sum(p.numel() for p in model.parameters())

        # --- Try moving a single sample to GPU to check memory ---
        dummy_input = torch.zeros([1, 5, 32, 32, 32]).to(device, non_blocking=True)
        try:
            model = model.to(device)
            output = model(dummy_input, **model_kwargs)
            loss = loss_fn(output, dummy_input)
            loss.backward()
            del model, output, dummy_input
        except RuntimeError as e:
            if "out of memory" in str(e):
                print(
                    f"Trial {trial.number} pruned due to OOM with single sample. Parameters: {param_count}"
                )
        ...
        print(f"Pruning trial {trial.number}: repeated OOM errors.")
        raise optuna.TrialPruned()
...
if __name__ == "__main__":
    ...
    del reserve_memory
    loss_fn = MSELoss()
    experiment_folder = os.path.abspath("experiments/cfno_2/experiment_5/results")
