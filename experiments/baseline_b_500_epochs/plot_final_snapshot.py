"""
Final-snapshot comparison plot for the Baseline B (500 epochs) experiment.

Runs a fresh jf1uids turbulent simulation (seed 1234, 128^3), block-averages
to LR, runs every trained model from the baseline_b_500 manifest, and produces
...
This script is the jf1uids-side companion to ``train_baseline_b_500.py``:
``run.sh`` invokes it after training. Ported from
``experiments/edsr_norm_skip/plot_final_snapshot.py``.

Usage
-----
    python experiments/baseline_b_500_epochs/plot_final_snapshot.py
    python experiments/baseline_b_500_epochs/plot_final_snapshot.py --regen
    python experiments/baseline_b_500_epochs/plot_final_snapshot.py \
        --manifest /export/scratch/jalegria/experiments/baseline_b_500_epochs_...
"""
...
def _plot_final_snapshot(states: dict, manifest: dict, output_dir: Path) -> None:
    ...
    _plot_state_grid(
        ...
        title="Baseline B (CFNO shift=8, MSE+spectral w_minor, skip) — 500 epochs",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "newest baseline_b_500_epochs_* under scratch."
        ),
    )
    parser.add_argument(
        "--regen",
    ...
    if args.manifest:
        ...
        manifest_dir = discover_latest(SCRATCH_BASE, "baseline_b_500_epochs_*")
