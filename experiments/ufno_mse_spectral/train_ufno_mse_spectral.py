"""
Train the best UFNO_2 model with MSE+spectral loss + the clip_p30 regime.

The best model from ``experiments/ufno_l1_spectral_unet/train_ufno_l1_spectral_unet.py``
is ``ufno_shift8_op2_drop01`` (lowest MSE @ x4 = 0.006370, best PSNR = 42.58 on
that experiment's benchmark; the op1_drop01 sibling has a degenerate
Spectral_MSE ≈ 1.05 and is excluded). The architecture (shift=8, skip=trilinear,
n_operator_blocks=2, dropout_rate=0.1) is reused verbatim.

This is a rewrite of that UFNO run but:

    - loss: ``MSESpectralLoss`` = ``nn.MSELoss() + spectral_weight * SpectralLoss``
      with the ``w_minor`` calibration weight from
      ``experiments/l1_spectral_weighting/calibration.json`` (the calibration
      source shared by all the CFNO baselines in this project).
    - regime: the ``clip_p30`` optimization hyper-parameters from
      ``experiments/comparing_best_models_mse/train_comparing_best_models_mse.py``
      — 500 epochs, ``AdamW(lr=1e-3, wd=1e-4)``,
      ``ReduceLROnPlateau(factor=0.8, patience=30)``, early-stop patience 40,
      **gradient clipping ``max_norm=0.8``**, ``batch_size=8 × grad_accum=8``
      (eff. 64), AMP off, Gaussian input noise σ=0.01, snapshot 79,
      eval x4 & x2.

Normalization is ON, reusing ``experiments/l1_spectral_weighting/normalization_stats.npz``
so results stay directly comparable to the other CFNO/EDSR baselines.
...
Bar charts / final-snapshot / spectra plots are produced by separate scripts
invoked from ``run.sh`` — no plotting code lives in this file.

Usage
-----
    bash experiments/ufno_mse_spectral/run.sh
    python experiments/ufno_mse_spectral/train_ufno_mse_spectral.py
    python experiments/ufno_mse_spectral/train_ufno_mse_spectral.py \
...
FOLDER_NAME = "ufno_mse_spectral_"
...
# ── Training hyper-parameters (clip_p30 regime from comparing_best_models_mse)
...
# ── UFNO hyper-parameters (best run from ufno_l1_spectral_unet) ───────
...
RUN_NAME = "ufno_shift8_mse_spectral_w_minor_500e_clip_p50"
RUN_LABEL = "UFNO op=2 drop=0.1 (500e, clip=0.8, p=30)"
...
# =====================================================================
# Losses (SpectralLoss + MSESpectralLoss — CFNO baseline family)
# =====================================================================
...
def _build_run_config(spectral_weight: float) -> dict:
    """The single UFNO run (best config, clip_p30 regime)."""
    return {
        "model_type": "ufno",
        "name": RUN_NAME,
        "label": RUN_LABEL,
...
def _find_existing_run_dir(run_name: str) -> Path | None:
    """Search existing ``ufno_mse_spectral_*`` folders for a trained run.
...
def train_one_run(
    ...
    print(f"  op={run_config['n_operator_blocks']}  drop={run_config['dropout_rate']}")
...
def _write_manifest(output_dir: Path, run_config: dict) -> dict:
    ...
    manifest = {
        "experiment": "ufno_mse_spectral",
        "output_dir": str(output_dir),
        "benchmark_csv": "experiments/ufno_mse_spectral/benchmark_metrics.csv",
