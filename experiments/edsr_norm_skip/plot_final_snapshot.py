"""
Final-snapshot comparison plot for the EDSR norm + skip experiment.

Runs a fresh jf1uids turbulent simulation (seed 1234, 128^3), block-averages
to LR, runs every trained model from the EDSR norm+skip manifest, and produces
a final-snapshot state-grid comparison (HR | model(s) | Trilinear | LR).
States are cached to ``comparison_states.npy`` in the home experiment folder.

This script is the jf1uids-side companion to ``train_edsr_norm_skip.py``:
``run.sh`` invokes it after training. Ported from
``experiments/mse_spectral_weighting/plot_final_snapshot.py``.

Usage
-----
    python experiments/edsr_norm_skip/plot_final_snapshot.py
    python experiments/edsr_norm_skip/plot_final_snapshot.py --regen
    python experiments/edsr_norm_skip/plot_final_snapshot.py \
        --manifest /export/scratch/jalegria/experiments/edsr_norm_skip_...
"""
...
# =====================================================================
# Sim + state-grid helpers (ported from loss_ablation)
# =====================================================================
...
def _build_states(regen: bool, manifest: dict, states_npy: Path) -> dict:
    ...
    for entry in manifest["models"]:
        ...
        try:
            ...
            lr_t = torch.from_numpy(lr).unsqueeze(0).to(DEVICE)
            # EDSR is fixed-scale; build_model ignores upsample_factor for
            # supports_variable_scale=False entries, so passing it is harmless.
...
def _plot_final_snapshot(states: dict, manifest: dict, output_dir: Path) -> None:
    ...
    _plot_state_grid(
        ...
        title="EDSR (norm-on, x4): skip-connection comparison (off / on)",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "newest edsr_norm_skip_* under scratch."
        ),
    )
    parser.add_argument(
        "--regen",
    ...
    if args.manifest:
        ...
        manifest_dir = discover_latest(SCRATCH_BASE, "edsr_norm_skip_*")
