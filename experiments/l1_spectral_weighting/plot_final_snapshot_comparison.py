"""
Final-snapshot comparison plots for the ``l1_spectral_weighting`` experiment.
...
runs every model listed in the experiment manifest (CFNO shift=8, normalized,
trained with L1 + spectral loss at three spectral weights: w_minor, w_equal,
w_major), and produces two comparison figures:

    plot 1 — comparison_all_x4.png
        rows: HR | w_minor | w_equal | w_major | trilinear interp | LR
        Shows the effect of spectral-loss weight magnitude on the SR output.

    plot 2 — comparison_best_x4.png
        rows: HR | best (lowest val MSE) | trilinear interp | LR

All models are norm-on, so ``evaluation.manifest.build_model`` normalizes the
LR input with cached training statistics before inference and denormalizes the
SR output back to the raw scale.
...
Usage
-----
    python experiments/l1_spectral_weighting/plot_final_snapshot_comparison.py
    python experiments/l1_spectral_weighting/plot_final_snapshot_comparison.py \
        --regen
    python experiments/l1_spectral_weighting/plot_final_snapshot_comparison.py \
        --manifest /path/to/experiment_folder
"""
...
# ── Best-weight selection from benchmark CSV ──────────────────────────


def _best_weight_name(manifest: dict) -> str | None:
    """Return the weight_name of the lowest-MSE trained run.

    Returns None if the benchmark CSV is missing or no trained run matched.
    """
    csv_path = manifest["benchmark_csv"]
    if not csv_path.exists():
        print("  (benchmark_results.csv missing — skipping best selection)")
        return None
    df = pd.read_csv(csv_path)
    row_key = manifest["benchmark_row_key"]
    value_field = manifest["benchmark_row_value_field"]
    scored = []
    ...
    best = scored[0][1]
    print(
        f"  Best by MSE: {best['weight_name']} "
        f"(MSE={scored[0][0]:.5f})"
    )
    return best["name"]
...
def main() -> None:
    ...
    # Collect trained-run entries (skip trilinear) in manifest order.
    trained = [e for e in manifest["models"] if e["model_type"] != "trilinear"]

    # ── Plot 1: all weight variants ────────────────────────────────
    all_rows = [("HR target", states["hr"])]
    for entry in trained:
        name = entry["name"]
        if name in states:
            wn = entry.get("weight_name", entry["name"])
            all_rows.append((wn, states[name]))
    all_rows.append(("Trilinear", states["interp"]))
    all_rows.append(("LR", states["lr"]))
    _plot_state_grid(
        rows=all_rows,
        save_path=OUTPUT_DIR / "comparison_all_x4.png",
        title="CFNO shift=8 L1+spectral — x4: spectral weight comparison",
    )

    # ── Plot 2: best by val MSE ────────────────────────────────────
    best_name = _best_weight_name(manifest)
    best_rows = [("HR target", states["hr"])]
    if best_name and best_name in states:
        best_entry = next(
            e for e in trained if e["name"] == best_name
        )
        best_rows.append(
            (best_entry.get("weight_name", best_name), states[best_name])
        )
    best_rows.append(("Trilinear", states["interp"]))
    best_rows.append(("LR", states["lr"]))
    _plot_state_grid(
        rows=best_rows,
        save_path=OUTPUT_DIR / "comparison_best_x4.png",
        title="CFNO shift=8 L1+spectral — x4: best spectral weight (by val MSE)",
    )
