"""
Calibrate the spectral-loss weight against L1 on normalized data.

Measures the L1 and velocity-spectral loss on a trilinear-upsampled LR
prediction vs the HR target, on a few batches of the normalized training set.
The ratio ``L1 / spectral`` gives the weight that makes both terms contribute
...
Results are saved to ``calibration.json`` and printed; ``train_l1_spectral.py``
...
# ── Spectral loss (same as train_l1_spectral.py) ──────────────────────
...
def main():
    ...
    print("Measuring L1 and spectral loss on trilinear-upsampled LR …")
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            spec_total += spec
            n += 1
            print(f"  batch {n}/{N_BATCHES}: L1={l1:.6f}  spectral={spec:.6f}")

    l1_avg = l1_total / n
    spec_avg = spec_total / n
    w_eq = l1_avg / spec_avg if spec_avg > 0 else float("inf")
