...
results_folder = os.path.abspath("/export/home/jalegria/Thesis/turbulence_sr/experiments/edsr/experiment_1/evaluation") #directory of the experiment
models_folder = os.path.abspath("/export/data/jalegria/models/edsr")
...
results_csv_path = os.path.join(results_folder, "eval_results.csv")
...
for model_folder in subfolders:
    ...
    fno_cfg = model_config["edsr"]

    model = EDSR(
        input_channels=5,
        n_resblocks=fno_cfg["n_resblocks"],
        n_feats=fno_cfg["n_feats"],
        kernel_size=fno_cfg["kernel_size"],
        activation_f=fno_cfg["activation_f"],
    ).to(device)
    ...
    while current_batch_size >= min_batch_size:
        try:
            ...
            new_results.append(
                ...
                "val_loss": val_loss,
                "time": time_per_item,
                "res_blocks": fno_cfg['n_resblocks'],
                "features": fno_cfg['n_feats'],
                "kernel_size": fno_cfg['kernel_size'],
                "activation_function": fno_cfg['activation_f']
                }
            )
            break
