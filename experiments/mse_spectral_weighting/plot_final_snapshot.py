"""
Final-snapshot comparison plot for the MSE+spectral weighting experiment.

Runs a fresh jf1uids turbulent simulation (seed 1234, 128^3), block-averages
to LR, runs every trained model from the MSE+spectral manifest, and produces
...
This script is the jf1uids-side companion to ``train_mse_spectral.py``:
``run.sh`` invokes it after training. Ported from
``experiments/loss_ablation/train_loss_ablation.py`` (final-snapshot section).

Usage
-----
    python experiments/mse_spectral_weighting/plot_final_snapshot.py
    python experiments/mse_spectral_weighting/plot_final_snapshot.py --regen
    python experiments/mse_spectral_weighting/plot_final_snapshot.py \
        --manifest /export/scratch/jalegria/experiments/mse_spectral_weighting_...
"""
...
def _plot_final_snapshot(states: dict, manifest: dict, output_dir: Path) -> None:
    ...
    _plot_state_grid(
        ...
        title="CFNO shift=8 — x4: MSE + spectral loss, spectral-weight comparison",
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "newest mse_spectral_weighting_* under scratch."
        ),
    )
    parser.add_argument(
        "--regen",
    ...
    if args.manifest:
        ...
        manifest_dir = discover_latest(SCRATCH_BASE, "mse_spectral_weighting_*")
