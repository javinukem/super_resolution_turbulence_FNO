"""
Calibrate the spectral-loss weight against MSE on normalized data.

Measures the MSE and velocity-spectral loss on a trilinear-upsampled LR
prediction vs the HR target, on a few batches of the normalized training set.
The ratio ``MSE / spectral`` gives the weight that makes both terms contribute
equally at the start of training. Three weights are then derived:

    w_minor  = w_eq * 0.1   (spectral is a gentle nudge, MSE dominates)
    w_equal  = w_eq         (both terms balanced)
    w_major  = w_eq * 10    (spectral dominates, aggressive high-freq matching)

Results are saved to ``calibration.json`` and printed; ``train_mse_spectral.py``
reads this file automatically.

Usage
-----
    python experiments/mse_spectral_weighting/calibrate_weights.py
"""
...
OUTPUT_DIR = ROOT / "experiments" / "mse_spectral_weighting"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
CALIBRATION_JSON = OUTPUT_DIR / "calibration.json"

# Reuse the l1_spectral_weighting normalization stats so all
# weighting/ablation experiments share identical normalization.
...
def main():
    ...
    print("Measuring MSE and spectral loss on trilinear-upsampled LR …")
    with torch.no_grad():
        for i, batch in enumerate(loader):
            ...
            spec_total += spec
            n += 1
            print(f"  batch {n}/{N_BATCHES}: MSE={mse:.6f}  spectral={spec:.6f}")

    mse_avg = mse_total / n
    spec_avg = spec_total / n
    w_eq = mse_avg / spec_avg if spec_avg > 0 else float("inf")
    ...
    print(f"  Ratio MSE/spec  = {w_eq:.4f}")
    print(f"{'=' * 50}")
    print(f"  Recommended spectral weights:")
